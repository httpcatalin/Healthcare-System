...
export function InvoiceProcessor() {
	...
	const [committed, setCommitted] = useState<{updated: any[]; purchaseOrderId?: string} | null>(null);
	...
	const total = useMemo(() => items.reduce((s, it) => s + (it.totalPrice || (it.quantity * it.unitPrice)), 0), [items]);
	...
	const extract = async () => {
		...
		try {
			...
			const res = await fetch(`${API_BASE}/invoice/extract`, { method: "POST", body: form });
	...
	const updateItem = (idx: number, patch: Partial<ParsedItem>) => {
		setItems((prev) => prev.map((it, i) => (i === idx ? { ...it, ...patch, totalPrice: (patch.unitPrice ?? it.unitPrice) * (patch.quantity ?? it.quantity) } : it)));
	};
	...
	return (
		<div className="space-y-6">
			<Card>
				<CardHeader>
					<CardTitle className="flex items-center gap-2"><FileText className="h-5 w-5" /> Invoice Processor</CardTitle>
					<CardDescription>Upload an invoice image/PDF, extract items, adjust if needed, select supplier, and save to DB.</CardDescription>
				</CardHeader>
				<CardContent className="space-y-4">
					...
					{extractedText && (
						<div>
							...
							<Textarea value={extractedText} onChange={(e) => setExtractedText(e.target.value)} rows={6} />
						</div>
					)}
									...
									<TableBody>
										{items.map((it, idx) => (
											<TableRow key={idx}>
												...
												<TableCell>${(it.totalPrice || it.quantity * it.unitPrice).toFixed(2)}</TableCell>
												<TableCell>
					...
					{committed && (
						<Alert>
							<AlertDescription>
								...
								{committed.purchaseOrderId ? ` Purchase Order: ${committed.purchaseOrderId}` : ""}
							</AlertDescription>
						</Alert>
					)}
				</CardContent>
