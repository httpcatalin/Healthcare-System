...
import { Users, DollarSign, Package, AlertTriangle, TrendingUp, FileText, Activity, Shield } from "lucide-react"
...
export function AdminDashboard() {
  ...
  const pendingOrders = purchaseOrders.filter((order) => order.status === "pending" || order.status === "approved")
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-secondary">{pendingOrders.length}</div>
            <p className="text-xs text-foreground">
              ${pendingOrders.reduce((sum, order) => sum + order.total, 0).toFixed(2)} total value
            </p>
          </CardContent>
        </Card>
          ...
          <AlertDescription>
            <div className="flex items-center justify-between">
              <div className="text-foreground">
                <strong>System Alerts:</strong> <span className="text-foreground">{criticalAlerts}</span> critical items
                need immediate attention, <span className="text-foreground">{lowStockAlerts}</span> items are running
                low.
              </div>
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={monthlySpend.map((spend, index) => ({ month: index + 1, spend }))}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              ...
              <div className="flex justify-between">
                <span>In Stock:</span>
                <Badge variant="default">{inventoryItems.filter((item) => item.status === "in-stock").length}</Badge>
              </div>
              <div className="flex justify-between">
                <span>Low Stock:</span>
